...
After the above preparations are completed, the control of the robotic arm begins with Python code. Here's an example of the myArm Controller 650 version.
* Import  pymycobot  library for MyArmC :
...
while i > 0:
    ...
    time.sleep(2)  # wait for 2 seconds	
    myarmc.set_tool_led_color(0, 255, 0)  #green light on
    time.sleep(2)  # wait for 2 seconds	
    i -= 1
```
